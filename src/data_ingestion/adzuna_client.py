...
class AdzunaClient:
    def __init__(self, stopwords: Optional[List[str]] = None):
        # -------- SAFE REQUEST SESSION WITH RETRIES ----------
        self.session = requests.Session()

        retries = Retry(
            total=5,                    # retry 5 times
            backoff_factor=1,           # wait 1,2,4,8...
    ...
    # ---------------------------------------------------------
    # SAFE JOB FETCHER WITH RETRIES & ERROR HANDLING
    # ---------------------------------------------------------
    def fetch_jobs(self, query: str, pages: int = 1, location: Optional[str] = None) -> List[AdzunaJob]:
        jobs: List[AdzunaJob] = []
        ...
        for page in range(1, pages + 1):
            ...
            # Non-200 responses (too many requests, downtime, etc.)
            if r.status_code != 200:
    ...
    # ---------------------------------------------------------
    # MAP API RESPONSE TO YOUR SCHEMA WITH SAFE DESCRIPTION
    # ---------------------------------------------------------
    def _map_to_job(self, entry: dict) -> Optional[AdzunaJob]:
        if not isinstance(entry, dict):
            return None
